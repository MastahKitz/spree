...
export async function assertProductDetails(page: Page, product: ProductData) {
  await expect.soft(page.getByRole('heading', { name: product.name, level: 1 })).toBeVisible();
  ...
  await expect.soft(page.getByRole('button', { name: 'Add to Cart' })).toBeVisible();
  // description
  await expect.soft(page.getByRole('heading', { name: 'Description', level: 2 })).toBeVisible();
  await expect.soft(page.getByText(product.description, { exact: true })).toBeVisible();
  // properties
  await expect.soft(page.getByRole('heading', { name: 'Properties', level: 2 })).toBeVisible();
  ...
  // details
  await expect.soft(page.getByRole('heading', { name: 'Details', level: 2 })).toBeVisible();
